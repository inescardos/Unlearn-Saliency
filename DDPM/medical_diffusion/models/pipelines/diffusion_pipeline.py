    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx:int):
        ...
        if self.global_step != 0 and self.global_step % self.sample_every_n_steps == 0:
            ...
            if hasattr(self.logger, 'log_image'):
                images_to_log = [img.permute(1, 2, 0).cpu().numpy() for img in sample_img]
                self.logger.log_image(key="samples", images=images_to_log[:16])  # Limit to 16 images

            # Save to disk only if we have a valid log directory
            if hasattr(self.logger, 'log_dir') and self.logger.log_dir is not None:
                try:
                    path_out = Path(self.logger.log_dir)/'images'
                    path_out.mkdir(parents=True, exist_ok=True)
                    save_image(
                        sample_img[:16],  # Save only first 16 samples
                        path_out/f'sample_{self.global_step:06d}.png',
                        nrow=4,
                        normalize=True,
                        padding=2
                    )
